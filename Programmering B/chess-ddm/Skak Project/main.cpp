...
{
    cout << "Move from " << char('A'+mv[1]) << 1+mv[0];
    cout << " to " << char('A'+mv[3]) << 1+mv[2] << endl;
}
...
// Calculate valid moves for a pawn piece
...
// Calculate valid moves for a bishop piece
...
// Calculate valid moves for a queen piece
...
//Calculate valid moves for a knight piece
vector<vector<int>> valid_moves_knight(vector<vector<int>>board, int pos_r, int pos_c, int color)
{
    vector<vector<int>> moves;
    ...
    int c = pos_c + 2;
    int r = pos_r+1;
    if( check_range(c) && check_range(r) && color * board[r][c] <= 0)
    {
        moves.push_back({pos_r,pos_c,r,c});
    }
...
//Calculate valid moves for the king piece
vector<vector<int>> valid_moves_king(vector<vector<int>>board, int pos_r, int pos_c, int color)
{
    vector<vector<int>> moves;
    ...
    //Move forward
    r = pos_r + 1;
    if(check_range(c) && check_range(r) && color * board[r][pos_c] <= 0)
    {
        moves.push_back({pos_r,pos_c,r,pos_c});
        cout << "test";
    }
    ...
    c=pos_c+1;
    r=pos_r;
    if(check_range(c) && check_range(r) && color * board[pos_r][c] <= 0)
    {
        moves.push_back({pos_r,pos_c,pos_r,c});
    }
...
float eval_pos(vector<vector<int>> board)
{
    ...
    for (unsigned int r=0; board.size(); r++)
    {
...
// Calculate computer move
    vector<int> calc_com_move(vector<vector<int>> board)
    {
        ...
        float score = eval_pos(board);                              // Evaluate current position
            ...
            //The brain to make the Queen moves
            current_pieces = find_pieces(board,COM * W_BISHOP);
            for(int k=0; k<current_pieces.size(); k++)
            {
                vector<vector<int>> move_list = valid_moves_queen(board,current_pieces[k][0],current_pieces[k][1], COM);
                COM_list.insert(COM_list.end(),move_list.begin(),move_list.end());
            }

            //The brain to make the Knight moves
            current_pieces = find_pieces(board,COM * W_KNIGHT);
            for(int k=0; k<current_pieces.size(); k++)
            {
                vector<vector<int>> move_list = valid_moves_knight(board,current_pieces[k][0],current_pieces[k][1], COM);
                COM_list.insert(COM_list.end(),move_list.begin(),move_list.end());
            }

            current_pieces = find_pieces(board,COM * W_KING);
            for(int k=0; k<current_pieces.size(); k++)
            {
                vector<vector<int>> move_list = valid_moves_king(board,current_pieces[k][0],current_pieces[k][1], COM);
                COM_list.insert(COM_list.end(),move_list.begin(),move_list.end());
            }
...
// COM MOVE - SLUT
    int main()
    {
        int win_size = 11*70;   // (8+2) x (8+2) squares of 70 pixels
